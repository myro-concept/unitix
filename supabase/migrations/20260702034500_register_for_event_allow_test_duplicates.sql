...
BEGIN
  ...
  -- Duplicate prevention in live mode only
  v_payment_mode := lower(trim(COALESCE(p_data->>'Payment Mode', '')));
  v_email := lower(trim(COALESCE(p_data->>'Email Address', p_data->>'email', p_data->>'Email', '')));

  IF v_payment_mode != 'test' AND v_email IS NOT NULL AND v_email != '' THEN
